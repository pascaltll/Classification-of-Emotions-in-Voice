...
class CNN(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.fc1 = nn.Linear(32 * 32 * 25, 128)  # Adjust based on input size
...
# Dataset for CNN (Mel-Spectrograms)
...
def main(cfg: DictConfig):
    wandb.init(project="emotion-classification-cnn", config=dict(cfg))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    model = CNN(num_classes=8)
    train_dataset = RAVDESSMelDataset(cfg.data.train_dir)
    val_dataset = RAVDESSMelDataset(cfg.data.val_dir)
    train_loader = DataLoader(train_dataset, batch_size=cfg.training.batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=cfg.training.batch_size)
    
    train_cnn(model, train_loader, val_loader, cfg, device)
    
    torch.save(model.state_dict(), os.path.join(cfg.model.save_path, 'cnn_model.pth'))
