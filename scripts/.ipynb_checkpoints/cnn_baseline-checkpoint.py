...
class CNN(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.fc1 = nn.Linear(32 * 32 * 39, 128)  # Verificar si 32*32*39 es correcto
        self.dropout = nn.Dropout(0.5)
        self.fc2 = nn.Linear(128, num_classes)
        self.relu = nn.ReLU()
    
    def forward(self, x):
#         print(f"Input shape: {x.shape}")  # [batch_size, 1, n_mels, time_steps]
        x = self.relu(self.conv1(x))
#         print(f"After conv1: {x.shape}")  # [batch_size, 16, n_mels, time_steps]
        x = self.pool(x)
#         print(f"After pool1: {x.shape}")  # [batch_size, 16, n_mels/2, time_steps/2]
        x = self.relu(self.conv2(x))
#         print(f"After conv2: {x.shape}")  # [batch_size, 32, n_mels/2, time_steps/2]
        x = self.pool(x)
#         print(f"After pool2: {x.shape}")  # [batch_size, 32, n_mels/4, time_steps/4]
        x = x.view(x.size(0), -1)
#         print(f"After flatten: {x.shape}")  # [batch_size, 32 * (n_mels/4) * (time_steps/4)]
